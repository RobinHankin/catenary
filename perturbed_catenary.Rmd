...
This is a second-order ODE with constraint $z(x_1)=z(x_2)=0$.  Note
that if we take the limit $\lambda\longrightarrow\infty$ we get

\begin{equation}
z'' = -3z'\tanh x,\qquad z(a)=0
\end{equation}

which has solution 

\begin{equation}
C(\arctan(\sinh x)  - \arctan(\sinh a) 
+ \sech(x)\tanh(x) - \sech(a)\tanh(a))
\end{equation}


and if $a=0$ we have $C(\arctan(\sinh x) + \sech(x)\tanh(x))$.  We
note that if $\lambda=\infty$ we can simultaneously satisfy the length
constraint and also $z(x_1)=z(x_2)=0$ by taking $C=0$.  The question
is, can we find another (non-infinite) value of $\lambda$ and
$z'(x_1)$ that satisfies the length constraint and also
$z(x_1)=z(x_2)=0$ for $C\neq 0$?
...
Lorenz <- function(t, state, parameters) {
    with(as.list(c(state, parameters)), {
        ...
        phi_x <- 1
        numerator <- (
            -phi_x + phi_y*(cosh(t) - sinh(t)*tanh(t))
            -(Z + phi)*ss
            -Zdash * tanh(t) * (1-3*lambda/cosh(t)^3 - 2*ss)
        )

        denominator <-  ss + lambda/cosh(t)^3
...
```{r usef,cache=TRUE}
v1 <- seq(from = 1, to = 2, by = 0.1)
v2 <- seq(from = -0.3, to = 0.3, by = 0.05)
...
plot(x, z,type="l")
plot(x, y0,type="l")
points(x, y0+z/100,type="l",col="red")
```
