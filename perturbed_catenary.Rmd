...
\underbrace{\phi(x,y)\sqrt{1+y'^2} +\frac{\mu z'y'}{\sqrt{1+y'^2}}}_{G}\\
&=& (z+\phi(x,y))\sqrt{1+y'^2}  + \frac{z'y'(y+\lambda+\mu)}{\sqrt{1+y'^2}}
\end{eqnarray}

So (writing $\lambda$ for $\lambda+\mu$):
...
\begin{eqnarray}
&\hphantom{+}&z''(\lambda\sech^3 x +\sech^2 x)\\
&-&  z'(3\lambda\sech^3x\tanh x + 2\sech^2\tanh x)\\
&+& (\phi_x + \phi_y\sinh x)\tanh x + \textcolor{red}{\phi\sech^2 x} \\
&{}& =\phi_y\cosh x
\end{eqnarray}
...
Lorenz <- function(t, state, parameters) {
    with(as.list(c(state, parameters)), {
        ...
        numerator <- (
            phi_y * cosh(t)
            +Zdash * tanh(t) * ss * (3*lambda/cosh(t) + 2)
            -(  (phi_x + phi_y*sinh(t))*tanh(t) + phi/cosh(t)^2)
        )
...
```{r usef, cache=TRUE}
v1 <- seq(from = 2, to = 5, by = 0.01)
v2 <- seq(from = -5, to = -2, by = 0.01)
...
```{r contbad}
contour(v1, v2, matrix(pmin(0.1,bad),length(v1),length(v2)),nlevels=10)
