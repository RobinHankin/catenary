...
\affil{\orgdiv{Computing Science and and Mathematics}, \orgname{University of Stirling}, \city{Stirling}, \postcode{FK9 4LA}, \country{Scotland}}


\abstract{This short document presents a method for dealing with
  slight perturbations to functionals.  We take a functional $F$ for
  which the Euler-Lagrange equations have a known solution $y=y(x)$.
  We then perturb the functional to $F+\epsilon G$ for some some small
  value of $\epsilon$, and retaining only terms to first order, obtain
  expressions for $z=z(x)$ such that $y+\epsilon z$ minimizes
  $F+\epsilon G$.  Numerical methods may be used to obtain $z$.
  Three applications from different areas of mathematical physics are
  presented: one from optics, one from mechanics, and one from the
  theory of general relativity.  In each case the perturbation from
  the known system is an informative and interesting result of the
  analysis.
}
...
implies that the response scales proportionally.  We start with a
given functional $F$ and Euler-Lagrange equations
...
&+& z (F_{xyy'} + y'F_{yyy'} + y''F_{yy'y'}-F_{yy})\nonumber\\
...
catenary is distorted into a ``vee'' shape; the accurate symmetry of
figure~\ref{trop} about $x=0$ is a consistency check.
...
equations for a stationary point mass~\cite{hartle2003}.  The
Schwarzschild metric gives the squared interval $ds^2$ in terms of
...
Following~\cite{hankin2021}, the Euler-Lagrange equations yield
...
\begin{eqnarray}\label{elF}
\frac{d}{dx}\left(\di{F}{r'}\right) -\di{F}{r} &=& \frac{r^2r''- 2rr'^2 -r^3}{(r^2 +r'^2)^{3/2}}\\
\frac{d}{dx}\left(\di{G}{r'}\right) -\di{G}{r} &=&\label{elG}
...
Equation~\ref{elF} gives $r'' = r + 2r'^2/r$ in the unperturbed case
from which we recover the familiar Euclidean straight-line solution
from Newtonian physics: $r=r_0\sec(\theta-\theta_0)$.  The left hand
side of equation~\ref{factorized_eqn} is then
...
to be a nominal circle of radius $r_0 = \qty{1.5e11}{\m}$, and use
this as the unit of length.  If we take the Schwarzschild radius of
the Sun to be $\qty{3e3}{\m}$, we find that the nondimensionalised
value of $2M_\odot$ is $10^{-8}$, and the linear
approximation is reasonable.

\begin{figure}[h]
\centering % File showblack-1.pdf created by euler_lagrange_perturbed.Rmd
...
$2M_\odot\cdot 0.7886$, or about $\qty{2.4}{\km}$.  Further, we may
exploit the linearity of equation~\ref{factorized_eqn}.  If we replace
the Sun with the Earth (of Schwarzschild radius $\qty{8.87}{\mm}$) in
the physical setup, then the perturbation is $2M_\oplus\cdot 0.7886$,
or about $\qty{7}{\mm}$.  This result appears to be difficult to
obtain any other way: because $\qty{7}{mm}\sim\qty{5e-14}{AU}$ we
would need to solve equation~\ref{eulerstring} to 15 decimal places, a
demanding task.
...
submitted work.  The author has no relevant financial or non-financial
interests to disclose.  The author has no competing interests to
