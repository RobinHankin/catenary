...
\begin{equation}
\frac{d}{dx}\left[\di{}{y'}\left(0+
\frac{z'y'}{\sqrt{1+y'^2}} + \phi\sqrt{1+y'^2}\right)\right] =
\phi_y\sqrt{1+y'^2}
\end{equation}
...
\begin{equation}
\frac{d}{dx}\left[
\frac{z'}{(1+y'^2)^{3/2}} + \frac{\phi y'}{\sqrt{1+y'^2}}\right]
= \phi_y\sqrt{1+y'^2}
\end{equation}


Now formally write $y'=s$, just to emphasise that it is in fact a
constant:

\begin{equation}
\frac{d}{dx}\left[
\frac{z'}{(1+s^2)^{3/2}} + \frac{\phi s}{\sqrt{1+s^2}}\right]
= \phi_y\sqrt{1+s^2}
\end{equation}

\begin{equation}
\frac{d}{dx}\left[z' + s\phi(1+s^2) \right] = \phi_y(1+s^2)^2
\end{equation}

At this point we remind ourselves that $\frac{d\phi}{dx}=\phi_x + s\phi_y$:

\begin{equation}
z'' + s(1+s^2)(\phi_x + s\phi_y) = \phi_y(1+s^2)^2
\end{equation}
...
Suppose $\phi=ax+by$.  Then we have

\begin{equation}
z'' = (1+s^2)(a-sb)
\end{equation}

This has solution $z=A + Bx + \frac{1}{2}(1+s^2)(a-sb)x^2$ where $A$
and $B$ are constants of integration.

Now we take $s=0$ and $\phi=ay^2/2$:

\begin{equation}
z''(x) = ay = az
\end{equation}

[above there is a bit of a terminological knot.  We are kind of
identifying $z$ with $y$ but I'm sort of using $y$ in two distinct and
incompatible ways in different parts of the analysis]
