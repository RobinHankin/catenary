...
blackhole <- function(t, state, parameters) {
    with(as.list(c(state, parameters)), {
        ...
        dZdash <- 4*Zdash*tan(x) + Z*(1-2*tan(x)^2)  + (tan(x)^2-1)
...
try <- seq(from = 0, to = 2, by = 0.1)
bad <- sapply(try, blackbadness)
plot(try,bad)

optimize(blackbadness, interval=c(-1,3))
...
```{r showblack}
parameters <- c(lambda = 0)
state      <- c(Z = 0, Zdash = 4/3)  # initial conditions
...
r <- 1/cos(theta)
plot(NA, xlim=c(-1,1), ylim=c(-0.1,1), asp=1, axes=FALSE, xlab="",ylab="")
