...
\begin{equation}\label{zdashdashdido}
...
\begin{eqnarray}
\lim_{\epsilon\longrightarrow\infty}\left[\int_{-\frac{\sqrt{2}}{2}}^{\frac{\sqrt{2}}{2}}
\sqrt{1+\left({1+(y' + \epsilon z')^2}\right)}\,dx
-
\int_{-\frac{\sqrt{2}}{2}}^{\frac{\sqrt{2}}{2}}
\sqrt{1+\left({1+(y' + \epsilon z')^2}\right)}\,dx\right]
&=&
\int_{-\frac{\sqrt{2}}{2}}^{\frac{\sqrt{2}}{2}}
\di{}{\epsilon}
...
We solve equation \ref{zdashdashdido} using the same numerical method as
for the straight line.

```{r didoODEdefiner}
library("deSolve")
dido <- function(t, state, parameters) {
    with(as.list(c(state, parameters)), {
        dZ <- Zdash
        x <- t

        dZdash <- 3*Zdash*x/(1-x^2) + lambda + 2/(1-x^2) + sqrt(2)/(1-x^2)^(3/2)

        dextra_length <- Zdash * (-x)
        list(c(dZ, dZdash, dextra_length))
    })
}
```


```{r solvedidoODE, cache=TRUE}

parameters <- c(lambda = 0)
state      <- c(Z = 0, Zdash = 0, extra_length = 0)  # initial conditions
times      <- seq(-sqrt(2)/2, sqrt(2)/2, by = 0.1)

out <- ode(y = state, times = times, func = dido, parms = parameters)
```
...
badness <- function(unknown, give=FALSE){
    ...
    times <- seq(from = -sqrt(2)/2, to = sqrt(1/2), len=5)
    parameters <- c(lambda = lambda)
    state <-  # initial conditions
        c(Z = 0, Zdash = Zdash_initial, extra_length=0) 
    out <- ode(y = state, times = times, func = dido, parms = parameters)
...
```{r usedidobadness, cache=TRUE}
(jj <- optim(c(0,0),badness, hessian = TRUE))
minbad <- jj$par

parameters <- c(lambda = minbad[1])
state <- c(Z = 0, Zdash = minbad[2], extra_length = 0)
times <- seq(-sqrt(2)/2, sqrt(2)/2, by = 0.1)
# initial conditions
out <- ode(y = state, times = times, func = dido, parms = parameters)
head(out)
...
```{r usef, cache=TRUE}
v1 <- seq(from = -5, to = -3, len=15)
v2 <- seq(from = -1, to =  1, len=15)
...
```{r calculate_optimized_didoODE, cache=TRUE}

parameters <- c(lambda = jj$par[1])
state      <- c(Z = 0, Zdash = jj$par[2], extra_length = 0)  # initial conditions
times      <- seq(-sqrt(2)/2, sqrt(2)/2,  len=40)
out <- ode(y = state, times = times, func = dido, parms = parameters)
```

```{r plot_optimized_didoODE}
x <- out[,1]
y <- out[,2]
plot(x,y,type='b')
grid()
...
```{r plot_optimized_didoODE2}
y0 <- sqrt(1-x^2) - sqrt(2)/2
plot(x,y0,asp=1, type='l')
polygon(cbind( x,y0), col= 'gray')
points(x,y0+y,asp=1, type='l',col='red')
```

```{r plot_optimized_didoODE3}
y0 <- sqrt(1-x^2) - sqrt(2)/2
plot(x,y0,asp=1, type='l')
polygon(cbind( x,y0), col= 'gray')
points(x,y0-y,asp=1, type='l',col='red')
...
```{r plot_optimized_didoODE4}
y0 <- sqrt(1-x^2) - sqrt(2)/2
plot(x,y0,asp=1, type='l')
polygon(c(x,rev(x)),c(y0,rev(y0+y)), col= 'gray')
points(x,y0+y,asp=1, type='l',col='red')
```
